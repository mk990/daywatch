...
// Hourly rollups pre-aggregate records per (hour, app, type) so long-range
// charts stay fast and survive raw-record pruning. They are upserted by a
// background ticker (full backfill at startup) and read by TimelineByClass
// for bucket sizes of an hour or more; the current partial hour is always
// aggregated live from records.
const rollupSchema = `
CREATE TABLE IF NOT EXISTS rollups_hourly (
    ...
    PRIMARY KEY (bucket, app, type)
);
`

func (s *Store) migrateRollups(ctx context.Context) error {
	_, err := s.pool.Exec(ctx, rollupSchema)
	return err
}
...
func (s *Store) UpdateRollups(ctx context.Context, since time.Time) error {
	...
	q := fmt.Sprintf(`
		INSERT INTO rollups_hourly
			(bucket, app, type, ok, warn, err, other, cnt, total_duration, p95, p99)
		SELECT date_trunc('hour', ts), app, type,
		...
		GROUP BY 1, 2, 3
		ON CONFLICT (bucket, app, type) DO UPDATE SET
...
func (s *Store) timelineFromRollups(ctx context.Context, app, typ string, origin, end time.Time, bucketMinutes int) ([]ClassBucket, error) {
	q := fmt.Sprintf(`
		WITH agg AS (
			...
			WHERE bucket >= $2 AND bucket < $4 AND bucket < date_trunc('hour', now())
			  AND ($1 = '' OR type = $1) AND ($5 = '' OR app = $5)
			UNION ALL
	...
	rows, err := s.pool.Query(ctx, q, typ, origin,
		fmt.Sprintf("%d minutes", bucketMinutes), end, app)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
